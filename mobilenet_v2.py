...
def inference(images, keep_probability, phase_train=True, bottleneck_layer_size=128, weight_decay=0.0, reuse=None):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                        ...
                        normalizer_fn=slim.batch_norm,
                        normalizer_params=batch_norm_params):
        with tf.variable_scope('mobilenetv2', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                                is_training=phase_train):

                #logits, end_points = mobilenet_v1(images,is_training=phase_train,depth_multiplier=1.0,num_classes=1000,prediction_fn=False,dropout_keep_prob=keep_probability)
                logits, end_points = mobilenet_v2_cls(images, num_classes=1000,dropout_keep_prob=keep_probability, is_training=phase_train,min_depth=8,depth_multiplier=1.0, conv_defs=None, prediction_fn=False, reuse=None, scope='MobilenetV2')
                
                net = slim.fully_connected(logits, bottleneck_layer_size, activation_fn=None, 
