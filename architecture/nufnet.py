...
def expand(inputs, num_outputs):
    with tf.variable_scope('expand'):
        ...
        e5x5 = slim.conv2d(inputs, num_outputs, [3, 3], padding='SAME', scope='3x3-1')
        e5x5 = slim.conv2d(e5x5, num_outputs  , [3, 3], padding='SAME', scope='3x3-2')

        e7x7 = slim.conv2d(inputs, num_outputs, [3, 3], padding='SAME', scope='3x3-3')
        e7x7 = slim.conv2d(e7x7, num_outputs  , [3, 3], padding='SAME', scope='3x3-4')
        e7x7 = slim.conv2d(e7x7, num_outputs  , [3, 3], padding='SAME', scope='3x3-5')
		
    return tf.concat([e1x1, e3x3, e5x5, e7x7], 3)

def inference(images, keep_probability, phase_train=True, bottleneck_layer_size=512, weight_decay=0.00004, reuse=None):
    batch_norm_params = {
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.variable_scope('nufnet', [images], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                                is_training=phase_train):
                net = slim.conv2d(images, 64, [7, 7], stride=2, scope='conv1')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool1')  
                net = fire_module(net, 16, 16, scope='fire1') 
                net = fire_module(net, 16, 16, scope='fire2')
                net = fire_module(net, 32, 32, scope='fire3')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool2')
                net = fire_module(net, 32, 32, scope='fire4') 
                net = fire_module(net, 32, 32, scope='fire5')
                net = fire_module(net, 64, 64, scope='fire6')
                net = slim.max_pool2d(net, [3, 3], stride=2, scope='maxpool3')
                net = fire_module(net, 64, 64, scope='fire7')
                net = fire_module(net, 64, 128, scope='fire8')
